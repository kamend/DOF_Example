...
float height = 768.0; //texture height
...
float ndofdist = 5.0; //near dof blur falloff distance
...
//float maxblur = 1.2; //clamp value of max blur (0.0 = no blur,1.0 default)

float threshold = 0.8; //highlight threshold;
float gain = 0.7; //highlight gain;
...
float bdepth(vec2 coords) //blurring depth
{
	...
	for( int i=0; i<9; i++ )
	{
		float tmp = texture2DRect(bgl_DepthTexture, coords + offset[i]).r;
		d += tmp * kernel[i];
	}
...
vec3 color(vec2 coords,float blur) //processing the sample
{
	...
	col.r = texture2DRect(bgl_RenderedTexture,coords + vec2(0.0,1.0)*texel*fringe*blur).r;
	col.g = texture2DRect(bgl_RenderedTexture,coords + vec2(-0.866,-0.5)*texel*fringe*blur).g;
	col.b = texture2DRect(bgl_RenderedTexture,coords + vec2(0.866,-0.5)*texel*fringe*blur).b;
...
void main()
{
    ...
    float depth = linearize(texture2DRect(bgl_DepthTexture,gl_TexCoord[0].xy).r);
	...
	if (autofocus)
	{
		fDepth = linearize(texture2DRect(bgl_DepthTexture,focus).x);
	}
	...
	float w = blur*maxblur+noise.x;
	float h = blur*maxblur+noise.y;
	...
	{
		col = texture2DRect(bgl_RenderedTexture, gl_TexCoord[0].xy).rgb;
	}
	...
	vec3 depthColor = texture2DRect(bgl_DepthTexture, gl_TexCoord[0].st).rgb;
